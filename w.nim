...
type
  ...
  KeccakContext = object
    q: array[25 * 8, byte]
    pt: int

template sizeDigest(ctx: KeccakContext): uint = 32
template rsize(ctx: KeccakContext): int = 136

proc finish(ctx: var KeccakContext, data: var openArray[byte]): uint =
...
proc finish(ctx: var KeccakContext): MDigest[256] =
